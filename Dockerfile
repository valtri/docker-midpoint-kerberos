...
RUN apt-get update && apt-get install -y --no-install-recommends krb5-admin-server krb5-kdc \
...
RUN wget -nv https://github.com/CESNET/kerberos-connector/releases/download/v${v}/debian-9.tar \
 && tar xf debian-9.tar \
...
RUN wget -nv https://raw.githubusercontent.com/CESNET/kerberos-connector/v${v}/samples/kerberos.xml \
 && sed -i kerberos.xml \
